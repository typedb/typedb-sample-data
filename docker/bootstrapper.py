...
BOOTSTRAPPER_VERSION = "1.0.0"  # Will fail if the bootstrapper.yml is of a different version.
DEFAULT_BOOTSTRAPPER_YML = "https://raw.githubusercontent.com/vaticle/typedb-sample-data/master/bootstrapper.yml"
...
def _console_command(cmd, force_verbose=False):
    return _run_cmd(["typedb", "console", "--core=127.0.0.1:%d"%BOOTSTRAPPER_PORT] + cmd)

def _run_cmd(cmd, force_verbose=False):
    output_to = None if (VERBOSE or force_verbose) else subprocess.DEVNULL
...
def main():
    try:
        config = load_config(DEFAULT_BOOTSTRAPPER_YML)
